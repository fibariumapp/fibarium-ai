    ...
    def _execute_query(
        ...
        try:
            ...
            try:
                ...
                return loop.run_until_complete(self._execute_query_async(query))
            
            # Если event loop запущен, используем безопасный способ выполнения
            if loop.is_running():
                # Используем run_in_executor для выполнения в отдельном потоке
                import concurrent.futures
                with concurrent.futures.ThreadPoolExecutor() as pool:
                    future = pool.submit(lambda: asyncio.run(self._execute_query_async(query)))
